...
import { formatCurrency, formatTime, getStatusColor, getInitials } from "@/lib/utils";
...
export default function Dashboard() {
  ...
  const membersColumns = [
    ...
    },
    {
      ...
      accessorKey: (member: any) => (
        <div>
          <div className="text-sm text-gray-900">{member.plan?.name || "Unknown"}</div>
  ...
  const classesColumns = [
      ...
      accessorKey: (gymClass: any) => (
        <div className="flex items-center">
          <Avatar className="h-8 w-8 mr-2">
            ...
            <AvatarFallback>{gymClass.trainer?.firstName && gymClass.trainer?.lastName ? getInitials(`${gymClass.trainer.firstName} ${gymClass.trainer.lastName}`) : "T"}</AvatarFallback>
          </Avatar>
    ...
    },
    {
      ...
      accessorKey: (gymClass: any) => {
        ...
        const percentage = capacity > 0 ? Math.round((bookingsCount / capacity) * 100) : 0;
        
        return (
          <div className="flex items-center">
            <span className="text-sm text-gray-500 mr-2">{bookingsCount}/{capacity}</span>
  ...
  return (
    <div className="py-6 md:py-8 px-4 sm:px-6 lg:px-8">
      ...
      <div className="mt-8 grid grid-cols-1 gap-6 lg:grid-cols-3 animate-slide-in">
        {/* Recent Activity */}
        <Card className="lg:col-span-2">
          <CardHeader className="px-6 py-5 border-b border-gray-200 flex flex-row items-center justify-between">
            ...
            <Button variant="link" className="text-primary-600 hover:text-primary-700 font-medium text-sm p-0">
              {t('viewAll')}
            </Button>
          </CardHeader>
              ...
              <ul className="divide-y divide-gray-200">
                {activities.map((activity: any) => (
                  <ActivityItem
                    ...
                    avatarFallback={activity.member ? getInitials(`${activity.member.firstName} ${activity.member.lastName}`) : undefined}
      ...
      <Card className="mt-8 animate-slide-in">
        <CardHeader className="px-6 py-5 border-b border-gray-200 flex flex-row items-center justify-between">
          <CardTitle className="text-lg font-medium">{t('recentMembers')}</CardTitle>
      ...
      <Card className="mt-8 animate-slide-in">
        <CardHeader className="px-6 py-5 border-b border-gray-200 flex flex-row items-center justify-between">
          <CardTitle className="text-lg font-medium">{t('todaysClasses')}</CardTitle>
          <Button variant="link" className="text-primary-600 hover:text-primary-700 font-medium text-sm p-0">
            {t('viewFullSchedule')}
          </Button>
        </CardHeader>
      ...
      <footer className="mt-8 text-center text-sm text-gray-500">
        <p>© {new Date().getFullYear()} GymPro Admin. {t('allRightsReserved')}.</p>
      </footer>
