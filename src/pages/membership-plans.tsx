...
export default function MembershipPlans() {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      await apiRequest(
        'DELETE',
        `/api/membership-plans/${selectedPlan.id}`,
        undefined,
      )
      toast({
        title: t('planDeleted'),
  ...
  const getDurationText = (plan: any) => {
    switch (plan.durationType) {
      case 'daily':
        return `${plan.duration} dia${plan.duration > 1 ? 's' : ''}`
      case 'weekly':
        return `${plan.duration} semana${plan.duration > 1 ? 's' : ''}`
      case 'monthly':
        return `${plan.duration} mes${plan.duration > 30 ? 's' : ''}`
      case 'quarterly':
        return `${plan.duration} cuatrimestre ${plan.duration > 90 ? 's' : ''}`
      case 'annual':
        return `${plan.duration} año${plan.duration > 365 ? 's' : ''}`
      default:
      ...
      <Dialog open={isFormOpen} onOpenChange={setIsFormOpen}>
        <DialogContent className='sm:max-w-[800px] max-h-[90vh] overflow-y-auto'>
          <DialogHeader>
            <DialogTitle>
              {selectedPlan
                ? t('editMembershipPlan')
                : t('addNewMembershipPlan')}
            </DialogTitle>
          </DialogHeader>
          <PlanForm
